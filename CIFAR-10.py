"""
FLGuardian (CIFAR-10): layer-wise clustering defense on real updates from CIFAR-10.

What this script does
---------------------
- Loads CIFAR-10 train/test.
- Partitions train set across M clients (Dirichlet non-IID).
- Each client trains locally (with optional label-flip attack).
- FLGuardian detects malicious clients via layer-wise clustering (cosine + euclidean).
- Aggregates only trusted updates and updates the global model.
- Reports detection metrics and test accuracy each round.
"""
...
# Try importing sklearn and torchvision
try:
...
try:
    ...
    TV_AVAILABLE = False
    raise RuntimeError("torchvision is required for CIFAR-10. Install with: pip install torchvision") from e
...
def pairwise_euclidean_distances(vectors: List[torch.Tensor]) -> np.ndarray:
    ...
    return np.sqrt(d2, dtype=np.float64).astype(np.float32)
...
def two_means_1d(values: np.ndarray, iters: int = 25) -> np.ndarray:
    ...
    for _ in range(iters):
        ...
        labels = new_labels
        if (labels == 1).any():
            c2 = values[labels == 1].mean()
        if (labels == 0).any():
            c1 = values[labels == 0].mean()
    return labels
...
class CIFARNet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64 * 8 * 8, 128)  # OK if 2 pool layers
...
# ----------------------- FLGuardian core ---------------------------------
class FLGuardian:
    def __init__(self, model_prototype: List[torch.Tensor], beta: float = 1.0,
                 top_k: Optional[int] = None, combine: Literal["intersection","union"] = "intersection",
        ...
        raw = np.array([self.beta ** l for l in range(self.layer_count)], dtype=np.float64)
    ...
    def _kmeans_2cluster_labels(self, values_1d: np.ndarray):
    ...
    def _larger_cluster_members(self, labels):
        ids0 = set(np.where(labels == 0)[0])
        ids1 = set(np.where(labels == 1)[0])
    ...
    def _get_candidate_benign_set_for_layer(self, layer_cosine, layer_euclid):
        ...
        vec_c = Xc.sum(axis=1)
        vec_e = Xe.sum(axis=1)
        labels_c = self._kmeans_2cluster_labels(vec_c)
        labels_e = self._kmeans_2cluster_labels(vec_e)
        cand_c = self._larger_cluster_members(labels_c)
        cand_e = self._larger_cluster_members(labels_e)
    ...
    def select_topk(self, scores, num_clients):
        if self.top_k is None:
            sel = [i for i in range(num_clients) if scores[i] > 0]
            return sel if sel else list(range(num_clients))
    ...
    def aggregate_mean(self, client_updates, selected):
        ...
        agg = []
        for l in range(self.layer_count):
            stack = torch.stack([client_updates[i][l].detach().cpu() for i in selected])
    ...
    def run_one_round(self, client_updates):
        ...
        diag = dict(benign_sets=benign_sets, scores=scores, selected_clients=selected)
        return agg, diag

# ------------------- CIFAR-10 data & partitioning ------------------------
def build_cifar10_loaders(batch_size=64, num_clients=10, dirichlet_alpha=0.5, seed=0):
    ...
    client_loaders = {i: DataLoader(Subset(train_ds, client_indices[i]),
...
# ----------------------- Train & Eval helpers ----------------------------
def params_to_list(model): return [p.detach().cpu().clone() for p in model.parameters()]
def apply_update_to_model(model, update, lr=1.0):
    with torch.no_grad():
        for p, u in zip(model.parameters(), update):
            p.add_(u.to(p.device, p.dtype), alpha=lr)
def delta_from_local(global_params, local_model):
...
if __name__ == "__main__":
    ...
    DIR_ALPHA = 0.9
    BATCH_SIZE = 64
    MALICIOUS_IDS = {2,10,14,18}
    LOCAL_POISON = True
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    client_loaders, test_loader = build_cifar10_loaders(BATCH_SIZE, M, DIR_ALPHA)
    global_model = CIFARNet().to(device)

    guardian = FLGuardian(model_prototype=params_to_list(global_model),
                          beta=2.0, top_k=TOP_K, combine="intersection", min_benign_layers=2)

    hist = {k: [] for k in ["accuracy","precision","recall","f1","TP","FP","TN","FN","test_acc"]}
    init_acc, _ = evaluate(global_model, test_loader, device)
    print(f"Initial model accuracy: {init_acc:.4f}")
    ...
    for r in range(R):
        ...
        client_updates = []
        for cid in range(M):
            local = CIFARNet().to(device)
            local.load_state_dict(global_model.state_dict(), strict=True)
            train_one_epoch(local, client_loaders[cid], device, poison=(cid in MALICIOUS_IDS and LOCAL_POISON))
        ...
        print(f"Pred malicious: {sorted(pred_mal)} | GT: {sorted(MALICIOUS_IDS)}")
        print(f"Round Metrics — Acc={m['accuracy']:.3f}, Prec={m['precision']:.3f}, Rec={m['recall']:.3f}, F1={m['f1']:.3f}")
        print(f"Global model acc: {test_acc:.4f}")

    def avg(x): return float(np.mean(x)) if len(x) else 0.0
    print("\n====== Overall Averages ======")
    print(f"Accuracy: {avg(hist['accuracy']):.3f}")
    print(f"Precision: {avg(hist['precision']):.3f}")
    print(f"Recall: {avg(hist['recall']):.3f}")
    print(f"F1: {avg(hist['f1']):.3f}")
